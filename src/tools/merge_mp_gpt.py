...
def get_model(model_type, pre_process, post_process):
...
def test(p):
    args = get_args()
    for t in range(args.target_tensor_model_parallel_size):
        ...
        mpu.initialize.set_tensor_model_parallel_rank(t)
        mpu.initialize.set_pipeline_model_parallel_rank(p)
        if mpu.initialize.is_pipeline_first_stage():
            pre_process = True
        else:
            pre_process = False
        if mpu.initialize.is_pipeline_last_stage():
            post_process = True
        else:
            post_process = False
        merged_model = get_model(args.model_type, pre_process, post_process)
        partitions = []
        tensor_model_chunk_size = args.tensor_model_parallel_size // args.target_tensor_model_parallel_size
        for r in range(tensor_model_chunk_size):
            mpu.initialize.set_tensor_model_parallel_world_size(args.tensor_model_parallel_size)
            rank = t * tensor_model_chunk_size + r
            ...
            #print('source:', mpu.initialize.get_tensor_model_parallel_rank(),mpu.initialize.get_pipeline_model_parallel_rank())
            checkpoint_name, iteration = get_parallel_checkpoint_name(args.load)
            model_ = get_model(args.model_type, pre_process, post_process)
...
def main():
    ...
    for p in range(args.pipeline_model_parallel_size): 
        for t in range(args.target_tensor_model_parallel_size):
            mpu.initialize.set_tensor_model_parallel_world_size(args.target_tensor_model_parallel_size)
            mpu.initialize.set_pipeline_model_parallel_world_size(args.pipeline_model_parallel_size)
            mpu.initialize.set_tensor_model_parallel_rank(t)
