...
class Pooler(MegatronModule):
    ...
    def forward(self, hidden_states, sequence_index=0, sequence_indexs=None):
        # hidden_states: [b, s, h]
        # sequence_index: index of the token to pool.

        if sequence_indexs is None:
            pooled = hidden_states[:, sequence_index, :]
            pooled = self.dense(pooled)
            pooled = torch.tanh(pooled)
        else:
            pooled = torch.ones((hidden_states.size()[0], hidden_states.size()[2]), device=hidden_states.device, dtype=hidden_states.dtype)
            for i in range(sequence_indexs.size()[0]):
                pooled[i, :] = hidden_states[i, sequence_indexs[i], :]
            pooled = self.dense(pooled)
            pooled = torch.tanh(pooled)
...
class TransformerLanguageModel(MegatronModule):
    ...
    def forward(self, enc_input_ids, enc_position_ids, enc_attn_mask,
                ...
                get_key_value=False, pooling_sequence_index=0, pooling_sequence_indexs=None,
        ...
        if self.post_process:
            if self.add_pooler:
                if pooling_sequence_indexs is None:
                    pooled_output = self.pooler(encoder_output,
                                                pooling_sequence_index)
                else:
                    pooled_output = self.pooler(encoder_output,
                                                sequence_indexs=pooling_sequence_indexs)
