...
class MultipleChoiceForGPT(MegatronModule):
    ...
    def __init__(self,
                 ...
                 pre_process=True,
                 post_process=True):
        super(MultipleChoiceForGPT, self).__init__()
        args = get_args()
        ...
        if args.encoder_attn_mask_type == 'causal':
            encoder_attn_mask_type_ = AttnMaskType.causal
        else:
            encoder_attn_mask_type_ = AttnMaskType.padding

        self.language_model, self._language_model_key = get_language_model(
            num_tokentypes=num_tokentypes,
            add_pooler=True,
            encoder_attn_mask_type=encoder_attn_mask_type_,
    ...
    def forward(self, model_input, position_ids, attention_mask, tokentype_ids=None, pooling_sequence_indexs=None):
        ...
        pooling_sequence_indexs = pooling_sequence_indexs.view(-1)
        lm_output = self.language_model(
            input_ids,
            position_ids,
            attention_mask,
            pooling_sequence_indexs=pooling_sequence_indexs,
